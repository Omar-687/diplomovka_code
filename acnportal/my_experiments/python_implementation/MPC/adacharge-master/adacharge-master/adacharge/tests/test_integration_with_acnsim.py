...
class TestAdaptiveChargingAlgorithmOfflineCaltechSingleDay(
    ...
    def setUpClass(cls) -> None:
        ...
        load_flattening_obj = [ObjectiveComponent(load_flattening)]
        cls.alg = AdaptiveChargingAlgorithmOffline(
            load_flattening_obj, solver=cp.ECOS, enforce_energy_equality=True
        )
        cls.alg.register_events(events)
        cls.sim = acnsim.Simulator(
            cn, cls.alg, events, start_time, period=period, verbose=False
...
class TestAdaChargeLoadFlattening(TestCase):
    def test_single_ev_feasible(self):
        ...
        alg = AdaptiveChargingAlgorithmOffline(
            load_flattening_obj, solver=cp.ECOS, enforce_energy_equality=True
        )
    ...
    def test_caltech_testcase_feasible(self):
        ...
        load_flattening_obj = [ObjectiveComponent(quick_charge),ObjectiveComponent(load_flattening)]
        # alg = adacharge_load_flattening(solver=cp.ECOS)
        alg = AdaptiveSchedulingAlgorithm(load_flattening_obj,solver=cp.ECOS)
    ...
    def test_external_signal(self):
        ...
        load_flattening_obj = [ObjectiveComponent(load_flattening(external_signal=external_sig))]
        alg = AdaptiveChargingAlgorithmOffline(load_flattening_obj,solver=cp.ECOS)
        alg.register_events(events)
        sim = acnsim.Simulator(cn, alg, events, start_time, period=period, verbose=False)
        alg.solve()
        sim.run()
